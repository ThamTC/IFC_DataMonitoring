...
const dbController = {
  ...
  update: async (req, res) => {
    try {
      ...
      const userUpdated = await db.GS_UserIFC.update(
  ...
  getRole: async (req, res) => {
    ...
    try {
      const resData = await db.GS_RolePermission.findOne({
  ...
  delete: async (req, res) => {
    try {
      const resData = await db.GS_UserIFC.destroy({
  ...
  createPermission: async (req, res) => {
    ...
    try {
      const found = await db.Permissions.findOne({
        where: { name: permissioName },
        raw: true,
      });
      if (found) {
  ...
  updatePermission: async (req, res) => {
    ...
    try {
      const found = await db.GS_RolePermission.findOne({
        where: { userId: userId },
        raw: true,
      });
      if (found) {
        const resData = await db.GS_RolePermission.update(
          { permission: permission },
          { where: { userId: userId } }
        );
        return res.status(200).json(resData);
      }
      const resData = await db.GS_RolePermission.create({
        userId: userId,
        permission: permission,
      });
      return res.status(200).json(resData);
  ...
  updateRole: async (req, res) => {
    ...
    try {
      const resData = await db.GS_UserIFC.update(
        { role: data },
        { where: { id: id } }
      );
      await db.GS_RolePermission.update(
        { permission: null },
        { where: { userId: id } }
      );
      return res.status(200).json(resData);
  ...
  deleteRole: async (req, res) => {
    const id = req.body.id;
    try {
      const resData = await db.GS_Roles.destroy({
        where: { id: id },
        raw: true,
      });
      return res.status(200).json(resData);
  ...
  requestRole: async (req, res) => {
    try {
      return res
        .status(200)
        .json("Yêu cầu đã được gử đi, vui lòng chờ xác nhận phía Management");
    } catch (error) {
  ...
  uploadImages: async (req, res) => {
    ...
    try {
      ...
      fs.writeFileSync(
        __dirname + `/../public/assets/images/${fileName}.${ext}`,
        base64Data,
        "base64"
      );
